...
public class ForgotPassword(IUserReadRepository userReadRepository, IEmailService emailService) : IForgotPassword
{
    public async Task Execute(ForgotPasswordRequest request)
    {
        ...
        if (user != null)
        {
            await emailService.SendPasswordResetCode(to: user.Email, userName: user.Name, code: 123456);
        }
    }
}