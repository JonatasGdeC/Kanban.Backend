...
public class SubTaskRepository(KanbanDbContext context) : ISubTaskReadRepository, ISubTaskWriteRepository
{
    ...
    {
        return await context.SubTasks.AsNoTracking().Where(predicate: subTask => subTask.TaskId == taskId && subTask.TaskEntity.Column.Board.UserId == userId).ToListAsync();
    }
    ...
    {
        return await context.SubTasks.AsNoTracking().FirstOrDefaultAsync(predicate: subTask => subTask.Id == id && subTask.TaskEntity.Column.Board.UserId == userId);
    }
}
